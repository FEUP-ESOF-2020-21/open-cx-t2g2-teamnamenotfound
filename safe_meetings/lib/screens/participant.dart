  ...
  List<Widget> showConfs() {
    ...
    if (conferences != null) {
      for (int i = 0; i < conferences.length; i++) {
        ...
        if (conferences[i].isParticipant(usermail) &&
            conferences[i].hasntVoted(usermail)) {
          //isParticpant retorna true se o email do participante estiver na lista dada pela conferencia.
          conferencesToVote++;
          confs.add(
            FlatButton(
              onPressed: () {
      ...
      if (conferencesToVote == 0) {
        confs.add(Text('No conferences to evaluate!',
